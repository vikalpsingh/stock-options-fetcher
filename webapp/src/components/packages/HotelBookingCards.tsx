...
const hotelOptions = [
  { city: "Ujjain", bestFor: "Walking access, early darshan, short trip", link: "/go/booking-ujjain-hotels", icon: Landmark },
  { city: "Indore", bestFor: "Better hotels, airport access, family comfort", link: "/go/booking-indore-hotels", icon: BedDouble },
  { city: "Bhopal", bestFor: "Extended MP trip, Omkareshwar/Sanchi add-on", link: "/go/booking-bhopal-hotels", icon: Building2 },
];
      ...
      {hotelOptions.map(({ icon: Icon, ...option }) => (
        <Card key={option.city} className="h-full border-gold/35">
          <CardContent>
            ...
            <Link href={option.link} rel="sponsored nofollow" className="mt-6 inline-flex items-center gap-2 text-sm font-bold text-maroon">
