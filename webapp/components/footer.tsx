  ...
  return (
    <footer className="pattern-mandala bg-[#211615] pb-20 text-stone-300 md:pb-0 print:hidden">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 sm:px-6 md:grid-cols-2 lg:grid-cols-4 lg:px-8">
        <div><div className="font-serif text-2xl font-bold text-white">{portal.brand}</div><p className="mt-1 text-xs font-bold uppercase tracking-widest text-gold">{portal.tagline}</p><p className="mt-4 max-w-sm text-sm leading-6 text-stone-400">{portal.focusNotice}</p></div>
        <div><p className="mb-4 text-xs font-bold uppercase tracking-widest text-gold">{copy?.footerPlan || "Plan"}</p><div className="space-y-3 text-sm"><Link className="block hover:text-white" href={href(locale === "en" ? "/plan-and-book" : "/plan-my-trip")}>{copy?.planTrip || "Plan my trip"}</Link><Link className="block hover:text-white" href={href("/stay-guide")}>{copy?.compareStays || "Compare stays"}</Link><Link className="block hover:text-white" href={href("/itineraries")}>{copy?.itineraries || "Itineraries"}</Link>{locale === "en" && <Link className="block hover:text-white" href="/ujjain-kumbh-2028/packages">Ujjain packages</Link>}</div></div>
      ...
      <div className="border-t border-white/10 px-4 py-5 text-center text-xs leading-5 text-stone-500">
        ...
        <p className="mt-2">© 2026 IndianKumbh · <Link href="/privacy" className="hover:text-white">Privacy</Link> · <Link href="/disclaimer" className="hover:text-white">Disclaimer</Link></p>
      </div>
    </footer>
  );
}
